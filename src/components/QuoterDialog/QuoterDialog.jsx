...
const neumorphicStyle = {
  ...
  boxShadow:
    "6px 6px 12px rgba(0, 0, 0, 0.1), -6px -6px 12px rgba(255, 255, 255, 0.2)",
  borderRadius: "12px",
};
    ...
    <Autocomplete
      ...
      ListboxProps={{
        sx: {
          ...
          boxShadow: "0 8px 30px rgba(0,0,0,0.15)",
          "& li:hover": {
            backgroundColor: "rgba(75,156,95,0.18)",
...
const QuoterNeoGlassProPanels = () => {
  ...
  // Validación nombre para solo letras y espacios
  const validateNombre = (nombre) => /^[a-zA-ZÀ-ÿ\s]*$/.test(nombre);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setNotification({
          open: true,
          message: "Error al cargar datos de API local.",
          severity: "error",
        });
      }
    };
  ...
  const handleAddArticle = () => {
    if (detalleEnvio.cantidad && detalleEnvio.tipo) {
      setArticulos([
        ...articulos,
        { ...detalleEnvio, id: Date.now() },
      ]);
      setDetalleEnvio({
        tipo: null,
        cantidad: "",
  ...
  // Actualizar nombre cliente con validación
  const handleNombreChange = (e) => {
    const val = e.target.value;
    if (val === "" || validateNombre(val)) {
      setForm({ ...form, nombreCliente: val });
    }
  };

  // Actualizar correo cliente con validación
  const handleCorreoChange = (e) => {
    const val = e.target.value;
    setForm({ ...form, correoCliente: val });
  };
  return (
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <RadioGroup
              ...
              onChange={(e) =>
                setForm({ ...form, tipoCliente: e.target.value })
              }
            >
              <FormControlLabel
                value="fisica"
          ...
          <Grid item xs={12} md={4}>
            <TextField
              ...
              error={form.correoCliente && !validateCorreo(form.correoCliente)}
              helperText={
                form.correoCliente && !validateCorreo(form.correoCliente)
                  ? "Correo inválido"
                  : ""
              }
            />
          ...
          <Grid item xs={12} md={4}>
            <TextField
              ...
              onChange={(e) =>
                setForm({ ...form, recoleccion: e.target.value })
              }
            />
          </Grid>
          <Grid item xs={12} md={4}>
          ...
          <Grid item xs={12} md={4}>
            <TextField
              ...
              onChange={(e) =>
                setForm({ ...form, cobroEspecial: e.target.value })
              }
            />
          </Grid>
          <Grid item xs={12} md={4}>
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  onChange={(e) =>
                    setForm({ ...form, domicilio: e.target.checked })
                  }
                />
              }
              label="A domicilio"
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={5}>
            <ModernBulkAutocomplete
              ...
              options={classBulk}
              onChange={(val) =>
                setDetalleEnvio({ ...detalleEnvio, tipo: val })
              }
            />
          </Grid>
          <Grid item xs={6} md={2}>
            <TextField
              ...
              onChange={(e) =>
                setDetalleEnvio({ ...detalleEnvio, cantidad: e.target.value })
              }
            />
          </Grid>
          <Grid item xs={6} md={4}>
            <TextField
              ...
              onChange={(e) =>
                setDetalleEnvio({ ...detalleEnvio, contiene: e.target.value })
              }
            />
          </Grid>
              ...
              control={
                <Checkbox
                  ...
                  onChange={(e) =>
                    setDetalleEnvio({
                      ...detalleEnvio,
                      usarMedidas: e.target.checked,
                    })
                  }
                />
              }
              label="Usar medidas volumétricas"
            />
          </Grid>
            ...
            <>
              <Grid item xs={4} md={2}>
                <TextField
                  ...
                  onChange={(e) =>
                    setDetalleEnvio({ ...detalleEnvio, largo: e.target.value })
                  }
                />
              </Grid>
              <Grid item xs={4} md={2}>
                <TextField
                  ...
                  onChange={(e) =>
                    setDetalleEnvio({ ...detalleEnvio, alto: e.target.value })
                  }
                />
              </Grid>
              <Grid item xs={4} md={2}>
                <TextField
                  ...
                  onChange={(e) =>
                    setDetalleEnvio({ ...detalleEnvio, ancho: e.target.value })
                  }
                />
              </Grid>
            </>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}>
                <AnimatedTotal label="IVA" value={form.iva} />
              </Grid>
              <Grid item xs={6}>
                <AnimatedTotal label="Ret. IVA" value={form.retIva} />
              </Grid>
              <Grid item xs={6}>
                <AnimatedTotal label="Total" value={form.total} />
              </Grid>
            </Grid>
          </Grid>
          <Grid item xs={12}>
...
const handleSendForm = async () => {
  try {
    const payload = { ...form, articulos };
    console.log("Datos a enviar al backend:", payload);

    const response = await fetch("http://localhost:3000/send-quotation", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });

    if (!response.ok) throw new Error("Error enviando la cotización");
